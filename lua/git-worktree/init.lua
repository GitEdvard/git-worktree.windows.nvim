...
local is_absolute = function(path)
    if string.find(path, ":/") == nil and string.find(path, ":\\") == nil then
        return false
    end
    return true
end
...
local function change_dirs(path)
    ...
    -- print("change_dirs, path: ".. path)
    -- print("change_dirs, worktree_path: ".. worktree_path)
    ...
    local job = Job:new({
        'git', 'worktree', 'list', on_stdout = function(_, data)
            ...
            if is_absolute(path) then
                ...
                worktree_path = git_worktree_root .. "/" .. path
                -- print("has_worktree path: " .. path)
                -- print("has_worktree path: " .. worktree_path)
                -- print("has_worktree data: " .. data)
...
local function create_worktree(path, branch, upstream, found_branch)
    ...
    if Path:new(path):is_absolute() then
        ...
        worktree_path = git_worktree_root .. "/" .. path
        -- print("worktree_path: " .. worktree_path)
    end
...
M.switch_worktree = function(path)
    status:reset(2)
    M.setup_git_info()
    -- print("switch_worktree, path: " .. path)
    has_worktree(path, function(found)
